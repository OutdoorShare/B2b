...
- **Pass platform fee to customer**: Admin settings → Policies → "Pass Service Fee to Customer" toggle. When on, the OutdoorShare service fee (e.g. 5%) is computed as a line item added on top of the rental total, shown explicitly in the price breakdown on the storefront booking page, and the exact `passthroughFeeCents` is sent to the payment-intent API so the platform collects the fee from the customer's payment rather than deducting it from the tenant's payout. When off (default), the platform fee is deducted from the tenant payout as before. `passPlatformFeeToCustomer` stored in `businessProfileTable`; `platformFeePercent` read from `tenantsTable` and returned by `GET /api/business`.
...
- **Per-Listing Embed Codes:** Each listing has an iframe-friendly embed URL at `/:slug/embed/listing/:id`. The embed page is a minimal, nav-free view showing the listing photo (with prev/next arrows for multiple images), category badge, title, price, location, condition, description, and a full-width "Book Now" CTA plus a "View full details" link — both open in a new tab so the parent page is unaffected. A "Powered by" footer shows the tenant's brand name and logo. The admin listing detail page (right sidebar) has an "Embed This Listing" card that displays the iframe snippet with a one-click copy button and a "Preview embed page" link. The embed route is registered in App.tsx at `/:slug/embed/listing/:id` without any layout wrapper, so no nav or StorefrontLayout is applied.
- **Split Payment Plans:** Tenants can enable split/delayed payment plans in Admin Settings → Policies tab. Configuration: deposit type (% of total or fixed $), deposit percent/amount, and days-before-pickup for the remaining charge. When enabled, customers on the storefront `book.tsx` see a toggle to pay just the deposit now (the Stripe PaymentIntent is created for the deposit amount only). The remaining balance is auto-charged by the scheduler on the configured due date using the card saved on file. Admin booking form has a "Split payment (deposit now)" mode; admin booking detail shows a payment plan status card with a "Charge Remaining Now" button. Customer `my-booking-detail.tsx` shows a payment plan breakdown card. Schema: `bookingsTable` fields `paymentPlanEnabled`, `splitDepositAmount`, `splitRemainingAmount`, `splitRemainingDueDate`, `splitRemainingStatus` (pending/charged/failed), `splitRemainingIntentId`, `splitRemainingChargedAt`; `businessProfileTable` fields `paymentPlanEnabled`, `paymentPlanDepositType`, `paymentPlanDepositFixed`, `paymentPlanDepositPercent`, `paymentPlanDaysBeforePickup`. Scheduler job `autoChargeRemainingBalances()` runs every 15 min, fires `POST /stripe/charge-remaining/:id` logic off-session, sends confirmation email via `sendSplitPaymentChargedEmail`.
