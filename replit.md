...
- **Rental Agreement System:** Dual-layer document architecture with operator contracts and platform (superadmin) agreements. Uses `{{token}}` Handlebars syntax for templates and typed "Contract Fields" for dynamic input. Combined PDF generation via v2 pipeline: template rendering → per-doc PDFKit generation → signature page → PDF merge with metadata stamping. Supports both template-based and uploaded PDF contracts. Custom operator acknowledgement items with per-contract or global scope. Platform agreements are force-included when a booking has an active protection plan (overrides operator opt-out). PDF body renderer handles markdown-style content: headings, bullet/numbered lists, horizontal rules, and inline formatting.
- **Protection Plan:** Category-based coverage fees managed by superadmin (`platform_protection_plans` table). Tenant-level toggle (`protectionPlanEnabled`) and optional/mandatory setting (`protectionPlanOptional`). Server-side price integrity enforcement prevents client-side manipulation. When active on a booking, platform legal agreements are automatically required regardless of operator preference.
