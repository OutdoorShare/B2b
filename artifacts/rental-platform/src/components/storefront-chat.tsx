...
import { MessageCircle, X, Send, ChevronDown, Loader2, Check, CheckCheck } from "lucide-react";
...
export function StorefrontChat({
  ...
  // Fetch thread + messages for this customer
  const fetchThread = useCallback(async () => {
    try {
      ...
      if (openThread.id !== activeThread?.id) {
        setActiveThread(openThread);
      }
    ...
    try {
      if (!activeThread) {
        ...
        const res = await fetch(`${BASE_URL}/api/chat/threads`, {
          method: "POST",
          headers,
          body: JSON.stringify({ customerEmail, customerName, subject: "Chat", body }),
        });
        if (res.ok) {
        ...
        const res = await fetch(`${BASE_URL}/api/chat/threads/${activeThread.id}/messages`, {
          method: "POST",
          headers,
          body: JSON.stringify({ senderType: "renter", senderName: customerName, body }),
        });
        if (res.ok) {
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(v => !v)}
        className="fixed bottom-5 right-20 z-40 w-13 h-13 rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-105 active:scale-95"
        style={{ width: 52, height: 52, backgroundColor: btnBg }}
            ...
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-white/70">
                  {activeThread?.status === "open" ? "● Online — typically replies quickly" : activeThread?.status === "closed" ? "Conversation closed" : "Start a conversation"}
                </p>
              </div>
            </div>
            <button
            ...
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-400">
                ...
                <p className="text-sm text-center">
                  Send a message to start a conversation with {companyName}
                </p>
              </div>
            )}
