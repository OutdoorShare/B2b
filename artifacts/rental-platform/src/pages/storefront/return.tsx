...
import { Camera, Upload, CheckCircle2, ImagePlus, X, AlertCircle, Loader2, PackageCheck } from "lucide-react";
...
export default function ReturnPage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`${BASE}/api/return/${token}`)
      .then(r => r.json())
      .then(data => {
        if (data.error) { setError(data.error); }
        else {
          setInfo(data);
          if (data.returnCompleted) {
            setDone(true);
            setSavedPhotos(data.returnPhotos);
          }
        }
        setLoading(false);
      })
      .catch(() => { setError("Failed to load return info"); setLoading(false); });
  }, [token]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const r = await fetch(`${BASE}/api/return/${token}/photos`, { method: "POST", body: formData });
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-gray-600">Loading return info…</p>
      </div>
    </div>
  );
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Return Photo Check</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-4">
            ...
            <p className="text-gray-600 text-sm mt-0.5">Hi {info.customerName} — thanks for returning!</p>
        ...
        {done ? (
          <div className="space-y-4">
            <div className="bg-white rounded-2xl shadow-sm border border-blue-100 p-6 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-1">Return Photos Submitted!</h3>
              <p className="text-gray-500 text-sm">
                Your {savedPhotos.length} photo{savedPhotos.length !== 1 ? "s" : ""} have been saved. The rental company will review the equipment condition.
              </p>
            </div>
            {savedPhotos.length > 0 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                <p className="text-sm font-semibold text-gray-700 mb-3">Your submitted return photos</p>
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
              <div className="flex items-start gap-3">
                <PackageCheck className="w-5 h-5 mt-0.5 flex-shrink-0 text-blue-600" />
                <div>
                  ...
                  <ul className="mt-2 space-y-0.5">
                    {["All sides — front, back, left, right", "Any scratches, dents, or marks", "All accessories and parts returned"].map(tip => (
                      <li key={tip} className="text-xs text-gray-500 flex items-center gap-1.5">
                        <span className="w-1 h-1 rounded-full inline-block flex-shrink-0 bg-blue-600" />
                        {tip}
                      </li>
                    ))}
                  </ul>
            ...
            {/* Upload area */}
            <div
              className="bg-white rounded-2xl shadow-sm border-2 border-dashed border-gray-200 p-8 text-center cursor-pointer transition-colors hover:border-blue-400 hover:bg-blue-50/30"
            ...
            <Button
              ...
              className="w-full py-6 text-base font-bold rounded-2xl bg-blue-700 hover:bg-blue-800 text-white"
            >
              {uploading ? (
                <><Loader2 className="w-5 h-5 animate-spin mr-2" />Uploading…</>
              ) : (
                <><Upload className="w-5 h-5 mr-2" />Submit {staged.length > 0 ? `${staged.length} Return Photo${staged.length !== 1 ? "s" : ""}` : "Return Photos"}</>
              )}
            </Button>
